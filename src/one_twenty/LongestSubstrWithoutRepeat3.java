...
public class LongestSubstrWithoutRepeat3 {
	...
	public static void main(String[] args) {
		System.out.println(new LongestSubstrWithoutRepeat3().lengthOfLongestSubstring("aabaab!bb"));
	}
	...
	private List<Part> splitPart(Part part) {
		//只要有某一个字符能够出现3次或以上，就可以把字符串分解成两个
	...
	private List<Part> doSplit(List<Integer> value, String s, Map<Character, List<Integer>> map) {
		...
		List<Part> resultList = new ArrayList<>();
		resultList.addAll(splitPart(new Part(s1, map1)));
		resultList.addAll(splitPart(new Part(s2, map2)));
		return resultList;
	}
